...
class DataLoad:
  ...
  def insert_user_access_hist(self, *data):
    ...
    if user_id:
      try:
        ...
        val = (str(uuid1()), user_id[0], logintime, Vsync, source_ip, source_mac, logon_mode, auth_mode, device_category, parent_group)
    ...
    if user_info:
      ...
      if not logout_time[0]:
        ...
        try:
          ...
          val = (str(uuid1()), user_id, user_name, syslogID, Vsync, policy, src_ip, dst_ip, src_port, dst_port, src_zone, dst_zone, protocal,request_type, host, referer, url_time)
  ...
  def vpn_user_login_fail(self, *data):
    ...
    try:
      ...
      val = (str(uuid1()), device_mac,device_name,username,MAC,ip_address,zone,access_type,datetime.utcfromtimestamp(time).strftime('%Y-%m-%d %H:%M:%S'))
